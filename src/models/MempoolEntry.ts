...
import { IMempoolEntry, MempoolEntrySerialized } from "./EntitiesInterface";
...
export class MempoolEntry implements IMempoolEntry {
  ...
  /**
   * To replace an entry, a new entry must have at least 10% higher maxPriorityFeePerGas and 10% higher maxFeePerGas than the existing Entry
   * Return true if the Entry can replace the existing Entry
   * @param existingEntry
   * @returns boolean
   */
  ...
  /**
   * Compare the Entry with another one
   * @param entry
   * @returns boolean
   */
  ...
  validateAndTransfromUserOp(): void {
    try {
