...
export class ReputationEntry implements IReputationEntry {
  ...
  get opsSeen(): number {
    const enlapsedHours = Math.floor((now() - this.lastUpdateTime) / 3600);
    const newRep = Math.floor(this._opsSeen * (1 - enlapsedHours * (1 / 24)));
  ...
  get opsIncluded(): number {
    ...
    const newRep = Math.floor(
      this._opsIncluded * (1 - enlapsedHours * (1 / 24))
    );
    return Math.max(newRep, 0);
  }
